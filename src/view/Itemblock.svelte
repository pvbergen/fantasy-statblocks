<script lang="ts">
    ...
    import type ItemBlockRenderer from "./itemblock";
    ...
    export let renderer: ItemBlockRenderer;
    ...
    $: $itemStore = item;
    const maxColumns =
        !isNaN(Number(item.columns ?? layout.columns)) &&
        Number(item.columns ?? layout.columns) > 0
            ? Number(item.columns ?? layout.columns)
            : 2;

    const itemColumnWidth = Number(
        `${item.columnWidth}`.replace(/\D/g, "")
    );
    const columnWidth =
        !isNaN(itemColumnWidth ?? layout.columnWidth) &&
        (itemColumnWidth ?? layout.columnWidth) > 0
            ? itemColumnWidth
            : 400;

    const canDice =
        plugin.canUseDiceRoller && (item.dice ?? plugin.settings.useDice);
    const canRender = item.render ?? plugin.settings.renderDice;
    ...
    setContext<Writable<Partial<Item>>>("item", itemStore);
    setContext<boolean>("dice", canDice);
    setContext<boolean>("render", canRender);
    setContext<ItemBlockRenderer>("renderer", renderer);
    ...
    const setColumns = () => {
        if (item.forceColumns ?? layout.forceColumns) {
            columns = maxColumns;
            observer.disconnect();
            return;
        }
    ...
    menu.addItem((item) => {
        item.setTitle("Copy YAML")
            ...
            .onClick(async () => {
                try {
                    await navigator.clipboard.writeText(stringifyYaml(item));
                    new Notice("Item YAML copied to clipboard");
                } catch (e: unknown) {
                    new Notice(
    ...
    $: name = slugify(item.name ?? "", "no-name");
