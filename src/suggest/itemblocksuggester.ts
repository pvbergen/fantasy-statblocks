...
export class ItemblockSuggester extends EditorSuggest<string> {
    ...
    getSuggestions(ctx: EditorSuggestContext) {
        ...
        switch (this._context) {
            ...
            case SuggestContext.Item: {
                suggestions = this.plugin.itemApi.getItemaryNames();
                break;
            }
            case SuggestContext.Layout: {
                suggestions = this.plugin.itemManager.getSortedLayoutNames();
                break;
            }
            case SuggestContext.Property: {
                const layout = this.plugin.itemManager.getLayoutOrDefault(
                    this._layout
                ).id;
                this._props = this.plugin.itemManager.getProperties(layout);
                suggestions = [
                    ...new Set([
                        ...STANDARD_FIELDS,
                        ...(this._props.keys() ?? [])
    ...
    onTrigger(
        ...
        if (/^(item|extends):/.test(line)) {
            this._context = SuggestContext.Item;
            const match = line.match(/^(item|extends):\s?(.*)\n?/);
            if (!match) return null;
            const [_, param, query] = match;

            if (
                Itemary.getItemaryNames().find(
         ...
         * only do this if not already in layout/item suggester
         */
        try {
            ...
            this._layout =
                obj.layout ?? this.plugin.itemManager.getDefaultLayout().name;
        } catch (e) {
            this._keys = [];
        }
